...
if (-not $BackendOnly -and $frontendPort -gt 0 -and (Test-Path -LiteralPath $webRoot)) {
    ...
    $pkgMgr = if ($cfg.Frontend.PackageManager -eq 'bun') { 'bun' } else { 'npm' }
    if ($cfg.Frontend.Kind -eq 'next') {
        Start-Process cmd.exe -ArgumentList @($cmdFlag, "$pkgMgr run dev -- -p $frontendPort -H 127.0.0.1") -WorkingDirectory $webRoot
    } else {
        Start-Process cmd.exe -ArgumentList @($cmdFlag, "$pkgMgr run dev -- --port $frontendPort --host 127.0.0.1") -WorkingDirectory $webRoot
    }
}
